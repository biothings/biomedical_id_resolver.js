import { DefaultResolvableator as Resolvableator } from '../src/validate/default_validator';
import IrresolvableIDResolverInputError from '../src/common/exceptions';

describe("Test Resolvableator Class", () => {
    describe("Test validateIfInputIsObject function", () => {
        test("raise exception if input is an array", () => {
            const test_data = ["123"];
            const vd = new Resolvableator(test_data);
